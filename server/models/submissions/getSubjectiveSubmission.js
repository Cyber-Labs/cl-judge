...
function getSubjectiveSubmission({ username, params, query }) {
  return new Promise((resolve, reject) => {
    const { contestId, submissionId } = params
    const { moderator } = query
    let queryString, queryArr
    if (moderator) {
      queryString = `SELECT question_id, contest_id, username, response, submission_time, feedback, score, judged 
      ...
      queryString = `SELECT question_id, contest_id, username, response, submission_time, feedback, 
...
module.exports = getSubjectiveSubmission
